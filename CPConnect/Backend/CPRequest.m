...
static NSString* pAPIProtocol = @"http";
static NSString* pAPIDomain = @"pkucada.org";
static NSString* pAPIPort = @"8089";
...
- (id)parseJsonResponse:(NSData *)data error:(NSError **)error {
    ...
    *error = inplaceError;
	...
	if ([result isKindOfClass:[NSDictionary class]]) {
		if ([result objectForKey:@"error"] || (error != nil && self.status_code >= 400)) {
			if ([result objectForKey:@"error"])
				*error = [self formError:self.status_code userInfo:result];
			else
				*error = [self formError:self.status_code userInfo:@{@"error": result}];
			return nil;
		}
	}
...
- (void)handleResponseData:(NSData *)data {
	if( [self eventHandlerCount:kCPRawBlockHandlerKey] > 0 ) {
		...
		if( self.error ) {
			if (self.error.code == 422 && [self.error.userInfo[@"error"] isKindOfClass:[NSString class]] && [self.error.userInfo[@"error"] isEqualToString:@"NotLoggedIn"]) {
				UIStoryboard *sb = [UIStoryboard storyboardWithName:@"MainStoryboard" bundle:[NSBundle mainBundle]];
				[[UIApplication sharedApplication].delegate.window.rootViewController presentModalViewController:[sb instantiateViewControllerWithIdentifier:@"UsersNavC"] animated:YES];
			}
			[self _reportError:self.error];
		}
		else {
